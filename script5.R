...
vars5 <- c("idauniq", "dhwork", "scworki", "iasinc", "scptra", "scworka", "disex", "chinhh")
vars6 <- c("idauniq", "DhWork", "scworki", "IaSInc", "scptra", "scworka", "DiSex", "chinhh")
vars7 <- c("idauniq", "DhWork", "scworki", "IaSInc", "scptra", "scworka", "DiSex", "chinhh")
vars8 <- c("idauniq", "dhwork", "scworki", "iasinc", "scprta", "scworka", "indsex", "chinhh")
vars9 <- c("idauniq", "dhwork", "scworki", "iasinc", "scprta", "scworka", "indsex", "chinhh")
...
# chinhh mapping 1 to Yes and 2 to No
usl <- subset(usl, !is.na(chinhh))
usl$chinhh <- ifelse(usl$chinhh == 1, "Yes", ifelse(usl$chinhh == 2, "No", usl$chinhh))
count(usl, chinhh)
...
# removing unwanted values from scptra
...
# removing unwanted values from scworka
usl <- subset(usl, !(scworka %in% c(-9, -8, -3, -1)))
count(usl, scworka)
...
# Models
# -------
model_scptra_ctrl <- lmer(iasinc ~ wave + scptra + disex + chinhh + scworka + (1 | idauniq), data = usl)
summary(model_scptra_ctrl)


# -------
# Extra
# -------
# Including Job Satisfaction as a Control
model_scptra_control <- lmer(iasinc ~ wave + scptra + scworka + (1 | idauniq), data = usl)
summary(model_scptra_control)

# Assessing Lag Effects
# usl$lag_scworka1 <- dplyr::lag(usl$scworka, 1)  # Job satisfaction lagged by 1 wave
usl$lag_scworka2 <- dplyr::lag(usl$scworka, 2)  # Job satisfaction lagged by 2 waves 
usl$lag_scworka3 <- dplyr::lag(usl$scworka, 3)  # Job satisfaction lagged by 2 waves 
usl$lag_scworka4 <- dplyr::lag(usl$scworka, 4)  # Job satisfaction lagged by 2 waves 
usl$lag_scworka5 <- dplyr::lag(usl$scworka, 5)  # Job satisfaction lagged by 2 waves 

# Lagged Model with Control (all waves)
model_scptra_lag <- lmer(iasinc ~ wave + lag_scworka2 + lag_scworka3 + lag_scworka4 + lag_scworka5 + (1 | idauniq), data = usl)
summary(model_scptra_lag)

# Lagged Model with Control (wave 1 only)
model_scptra_lag <- lmer(iasinc ~ wave + scptra + lag_scworka1 + scworka + (1 | idauniq), data = usl)
summary(model_scptra_lag)
